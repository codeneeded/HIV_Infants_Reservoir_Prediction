...
out.path <- 'C:/Users/axi313/Documents/HIV_Infants_Reservoir_Prediction/DC_Mixed_Model_Output/'
...
# Inner join the demographics and d_data dataframes based on PID and Age
dendritic <- inner_join(demographics, d_data, by = c("PID", "Age"))

dendritic <- as.data.frame(dendritic)
...
percentage_columns <- setdiff(names(dendritic), covariate_columns)
covariates <- dendritic[, covariate_columns]
...
percentage_columns <- setdiff(names(dendritic), covariate_columns)
percentage_dendritic <- dendritic[, percentage_columns]


# Set row names of covariates to unique_id
rownames(covariates) <- covariates$unique_id
rownames(percentage_dendritic) <- covariates$unique_id

# Transposing percentage_dendritic
percentage_dendritic_transposed <- t(percentage_dendritic)
...
v <- voom(percentage_dendritic_transposed, design, plot = TRUE)
...
# Extract results
results_with_voom <- topTable(fit_voom, number=50,coef = "GroupHEI")
...
print(volcano_plot)
ggsave(paste0(out.path,"HEIvsHEU_dendritic_Volcano.png"),bg='white',plot=volcano_plot)
...
sapply(dendritic[c("Viral Titre", "Age", "Sex", "PID","Group")], is.null)

# Check for NA values
sapply(dendritic[c("Viral Titre", "Age", "Sex", "PID","Group")], function(x) any(is.na(x)))
...
# Loop through each DC subset, ensuring column names are correctly handled
...
results_df <- results_df[results_df$Subset != "DCpanel_CD45_Raw_Counts", ]
...
viral_effect <- ggplot(viral_titre_effects, aes(x = reorder(Subset, Estimate), y = Estimate)) +
  ...
  coord_flip() +
  labs(title = "Effect of Viral Titre on DC Subsets", x = "DC Subset", y = "Effect Size") +
  theme_minimal()

ggsave(paste0(out.path,"Viral_Effect_dendritic_Coefficient_Plot.png"),bg='white',width=8, height=11.5,plot=viral_effect)
...
age_effect <- ggplot(age_effects, aes(x = reorder(Subset, Estimate), y = Estimate)) +
  ...
  geom_text(aes(label = Significance), vjust = -0.5, colour = "red") +  # Color the stars red
  coord_flip() +
  labs(title = "Effect of Age on DC Subsets", x = "DC Subset", y = "Effect Size") +
  theme_minimal()

ggsave(paste0(out.path,"Age_Effect_dendritic_Coefficient_Plot.png"),bg='white',width=8, height=11.5,plot=age_effect)
...
sex_effect <- ggplot(sex_effects, aes(x = reorder(Subset, Estimate), y = Estimate)) +
  ...
  coord_flip() +
  labs(title = "Effect of Being Male on DC Subsets", x = "DC Subset", y = "Effect Size Difference") +
  theme_minimal()
ggsave(paste0(out.path,"Sex_Effect_dendritic_Coefficient_Plot.png"),bg='white',width=8, height=11.5,plot=sex_effect)
...
HEI_effect <- ggplot(group_effects, aes(x = reorder(Subset, Estimate), y = Estimate)) +
  ...
  geom_text(aes(label = Significance), vjust = -0.5, colour = "red") +  # Color the stars red
  coord_flip() +
  labs(title = "Effect of HEI on DC Subsets", x = "DC Subset", y = "Effect Size Difference") +
  theme_minimal()

ggsave(paste0(out.path,"HIV_Effect_dendritic_Coefficient_Plot.png"),bg='white',width=8, height=11.5,plot=HEI_effect)
