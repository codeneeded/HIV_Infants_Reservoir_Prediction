...
out.path <- 'C:/Users/axi313/Documents/HIV_Infants_Reservoir_Prediction/NK_Mixed_Model_Output/'
...
percentage_columns <- setdiff(names(nkiller), covariate_columns)
covariates <- nkiller[, covariate_columns]
...
percentage_columns <- setdiff(names(nkiller), covariate_columns)
percentage_nkiller <- nkiller[, percentage_columns]


# Set row names of covariates to unique_id
rownames(covariates) <- covariates$unique_id
rownames(percentage_nkiller) <- covariates$unique_id

# Transposing percentage_nkiller
percentage_nkiller_transposed <- t(percentage_nkiller)
...
v <- voom(percentage_nkiller_transposed, design, plot = TRUE)
...
print(volcano_plot)
ggsave(paste0(out.path,"HEIvsHEU_nkiller_Volcano.png"),bg='white',plot=volcano_plot)
...
sapply(nkiller[c("Viral Titre", "Age", "Sex", "PID","Group")], is.null)

# Check for NA values
sapply(nkiller[c("Viral Titre", "Age", "Sex", "PID","Group")], function(x) any(is.na(x)))
...
results_df <- results_df[results_df$Subset != "NKpanel_CD45_Raw_Counts", ]
...
viral_effect <- ggplot(viral_titre_effects, aes(x = reorder(Subset, Estimate), y = Estimate)) +
  ...
  coord_flip() +
  labs(title = "Effect of Viral Titre on NK Subsets", x = "NK Subset", y = "Effect Size") +
  theme_minimal()

ggsave(paste0(out.path,"Viral_Effect_nkiller_Coefficient_Plot.png"),bg='white',width=8, height=11.5,plot=viral_effect)
...
age_effect <- ggplot(age_effects, aes(x = reorder(Subset, Estimate), y = Estimate)) +
  ...
  geom_text(aes(label = Significance), vjust = -0.5, colour = "red") +  # Color the stars red
  coord_flip() +
  labs(title = "Effect of Age on NK Subsets", x = "NK Subset", y = "Effect Size") +
  theme_minimal()

ggsave(paste0(out.path,"Age_Effect_nkiller_Coefficient_Plot.png"),bg='white',width=8, height=11.5,plot=age_effect)
...
sex_effect <- ggplot(sex_effects, aes(x = reorder(Subset, Estimate), y = Estimate)) +
  ...
  coord_flip() +
  labs(title = "Effect of Being Male on NK Subsets", x = "NK Subset", y = "Effect Size Difference") +
  theme_minimal()
ggsave(paste0(out.path,"Sex_Effect_nkiller_Coefficient_Plot.png"),bg='white',width=8, height=11.5,plot=sex_effect)
...
HEI_effect <- ggplot(group_effects, aes(x = reorder(Subset, Estimate), y = Estimate)) +
  ...
  geom_text(aes(label = Significance), vjust = -0.5, colour = "red") +  # Color the stars red
  coord_flip() +
  labs(title = "Effect of HEI on NK Subsets", x = "NK Subset", y = "Effect Size Difference") +
  theme_minimal()

ggsave(paste0(out.path,"HIV_Effect_nkiller_Coefficient_Plot.png"),bg='white',width=8, height=11.5,plot=HEI_effect)
